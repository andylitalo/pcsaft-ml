...
- SVR constructor uses `max_iter=10000`
- Gamma grid includes all 4 values: `['scale', 'auto', 0.01, 0.001]`
- `ConvergenceWarning` count captured and reported in summary artifact
- Grid boundary check implemented (boundary hits logged)
- Full `cv_results`_ saved to `model/saved/svm/cv_results.json`
- `scripts/train_step47.py` runs end-to-end with per-split and aggregate metrics
- Uses 10 repeated stratified outer splits
- Reports mean, std, and 95% intervals for epsilon_k comparison
- epsilon_k reported separately as primary benchmark target
- Best-run-only reporting explicitly avoided
- SVM and Ridge rows to be added to Esper comparison table in model_comparison.md
- Representation-bottleneck narrative updated
- R² heatmap and bottleneck comparison chart generated
- Smoke tests for SVM (3 tests) and Ridge (3 tests) — all passing
