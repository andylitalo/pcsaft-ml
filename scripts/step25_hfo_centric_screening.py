"""Step 25: HFO-Centric Screening — Full Pipeline.

Run complete blowing agent screening using HFO-1336mzz(Z) as reference:
1. Generate candidates via systematic enumeration
2. Predict PC-SAFT parameters via RF
3. Compute boiling points via teqp EOS
4. Compute SA scores
5. Apply HFO-centric filters (T_b, C=C, Cl-free, F>=2, SA<=4.5)
6. Rank by HFO parameter distance
7. Compute Henry's constant for top 50 filter-passing candidates
8. Save ranked results

Usage:
    python scripts/step25_hfo_centric_screening.py
"""
...
def print_warning_banner():
    ...
    print("\n" + "=" * 80)
    print("WARNING: OUT-OF-DOMAIN PREDICTION WORKFLOW")
    print("=" * 80)
    print("This workflow ranks OOD hypotheses using ML-predicted PC-SAFT parameters")
    ...
    print()
    print("Limitations:")
    print("  - RF model R² on ε/k: 0.27 (Esper test set)")
...
def main():
    """Run full HFO-centric screening pipeline."""
    ...
    print("\n[2/8] Predicting PC-SAFT parameters via Random Forest...")
    start = time.time()
    ...
    print("\n[5/8] Applying HFO-centric screening filters...")
    print("Filters:")
    ...
    filter_funnel.to_csv(output_dir / "hfo_filter_funnel.csv", index=False)
    print(f"\nFilter funnel saved to: {output_dir / 'hfo_filter_funnel.csv'}")

    # Stage 6: Rank by HFO parameter distance
    ...
    print("\n[8/8] Saving results...")
    output_path = output_dir / "hfo_centric_ranked.csv"
    filtered_df.to_csv(output_path, index=False)
    print(f"Saved {len(filtered_df)} ranked candidates to: {output_path}")

    # Print top 20
    print("\n" + "=" * 80)
    print("TOP 20 HFO-CENTRIC CANDIDATES")
    print("=" * 80)
    print()
